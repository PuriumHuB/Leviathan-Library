...
local F=b.NewRoundFrame(m,"Squircle",{
...
local J=(u.Icon and-28-i)or 0
local L=d("Frame",{
Size=UDim2.new(1,J,1,0),
...
d("TextLabel",{
AutomaticSize="Y",
Size=UDim2.new(1,A and-22 or 0,0,0),
TextWrapped=true,
TextXAlignment="Left",
RichText=true,
BackgroundTransparency=1,
...
local M=b.NewRoundFrame(m,"Squircle",{
Size=UDim2.new(1,0,0,0),
...
u.Closed=true
e(N,0.45,{Size=UDim2.new(1,0,0,-8)},
Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
e(M,0.55,{Position=UDim2.new(2,0,1,0)},
Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
task.wait(0.45)
N:Destroy()
end
...
task.spawn(function()
task.wait()
e(N,0.45,{Size=UDim2.new(1,0,0,M.AbsoluteSize.Y)},
Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
e(M,0.45,{Position=UDim2.new(0,0,1,0)},
Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()

if u.Duration then
F.Size=UDim2.new(0,M.DurationFrame.AbsoluteSize.X,1,0)
e(M.DurationFrame.Frame,u.Duration,
{Size=UDim2.new(0,0,1,0)},
...
ar=ae.InputChanged:Connect(function(aD)
if not al.Window.IsToggleDragging then
return
end
if
aD.UserInputType~=Enum.UserInputType.MouseMovement
and aD.UserInputType~=Enum.UserInputType.Touch
then
return
end
if aB then
...
local aE=math.abs(aD.Position.X-ay)
math.abs(aD.Position.Y-az)

if not aC and aE>8 then
aC=true
end

local aF=aD.Position.X-ay
local aG=math.max(2,math.min(aA+aF,au-at-2))

local b=math.clamp((aG-2)/(au-at-4),0,1)

local d,f,g=am:GetGlassFrame(b)
aq.Frame.Bar.Highlight.Glass.Image=d
aq.Frame.Bar.Highlight.Glass.ImageRectSize=f
aq.Frame.Bar.Highlight.Glass.ImageRectOffset=g

ad(aq.Frame,0.12,{
Position=UDim2.new(0,aG,0.5,0),
},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
end)
...
as=ae.InputEnded:Connect(function(aD)
if not al.Window.IsToggleDragging then
return
end
if
...
local aE=aq.Frame.Position.X.Offset
local aF=aE+at/2
local aG=aF>au/2
ax:Set(aG,true,false)
end
