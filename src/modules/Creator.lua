-- ╔══════════════════════════════════════════════════╗
-- ║        Leviathan UI — Creator Module             ║
-- ║  Premium design tokens · brighter palette        ║
-- ║  Enhanced typography · fluid animation helpers   ║
-- ╚══════════════════════════════════════════════════╝
...
-- WindUI forward-reference — set in Creator.Init
local WindUI
...
Creator = {
	...
	-- ── Leviathan: animation duration tokens ────────────────────
	Anim = {
		Instant = 0,
		Fast    = 0.10,
		Normal  = 0.20,
	...
	-- ── Leviathan: easing style presets ─────────────────────────
	Ease = {
	...
	-- ── Leviathan: semantic colour palette ──────────────────────
	Colors = {
		-- Status
		Red     = "#F04040",
		Orange  = "#F07020",
...
-- ═══════════════════════════════════════════════════════════════
--  Init  — called by WindUI on startup
-- ═══════════════════════════════════════════════════════════════
...
function Creator.DisconnectAll()
	for idx in next, Creator.Signals do
		local conn = table.remove(Creator.Signals, idx)
		if conn then conn:Disconnect() end
	end
end

-- ═══════════════════════════════════════════════════════════════
--  SafeCallback  — pcall wrapper with debug-notify support
-- ═══════════════════════════════════════════════════════════════
function Creator.SafeCallback(Function, ...)
	...
	if not Success then
		if WindUI and WindUI.Window and WindUI.Window.Debug then
			...
			warn("[ LeviathanUI DEBUG ] " .. tostring(Event))
...
function Creator.Gradient(stops, props)
	...
	for posStr, stop in next, stops do
		...
		if position then
			...
			table.insert(colorSeq, ColorSequenceKeypoint.new(position, stop.Color))
			table.insert(transSeq, NumberSequenceKeypoint.new(position, stop.Transparency or 0))
		end
	end
	table.sort(colorSeq, function(a, b) return a.Time < b.Time end)
	table.sort(transSeq, function(a, b) return a.Time < b.Time end)
	if #colorSeq < 2 then error("ColorSequence requires at least 2 keypoints") end
	local data = {
		Color        = ColorSequence.new(colorSeq),
		Transparency = NumberSequence.new(transSeq),
	}
	if props then
		for k, v in pairs(props) do data[k] = v end
	end
	return data
end

-- ═══════════════════════════════════════════════════════════════
--  Theme system  (100% API-compatible with WindUI)
-- ═══════════════════════════════════════════════════════════════
...
function Creator.GetThemeProperty(Property, Theme)
	local function getValue(prop, themeTable)
		...
		if typeof(value) == "table" and value.Color and value.Transparency then return value end
		if typeof(value) == "function" then return value(themeTable) end
		return value
	end
	...
	if value ~= nil then
		if typeof(value) == "string" and string.sub(value, 1, 1) ~= "#" then
			local ref = Creator.GetThemeProperty(value, Theme)
	...
	local fallback = Creator.ThemeFallbacks[Property]
	if fallback ~= nil then
		if typeof(fallback) == "string" and string.sub(fallback, 1, 1) ~= "#" then
			return Creator.GetThemeProperty(fallback, Theme)
		else
	...
	if value ~= nil then
		if typeof(value) == "string" and string.sub(value, 1, 1) ~= "#" then
			local ref = Creator.GetThemeProperty(value, Creator.Themes["Dark"])
			if ref ~= nil then return ref end
		else
			return value
		end
	...
	if fallback ~= nil then
		if typeof(fallback) == "string" and string.sub(fallback, 1, 1) ~= "#" then
			return Creator.GetThemeProperty(fallback, Creator.Themes["Dark"])
		else
			return getValue(Property, { [Property] = fallback })
		end
	end
...
function Creator.UpdateTheme(TargetObject, isTween, isTweenTarget, Duration, EasingStyle, EasingDirection)
	...
	if TargetObject then
		...
		for _, objData in pairs(Creator.Objects) do ApplyTheme(objData) end
	end
end
...
function Creator.UpdateLang(newLang)
	if newLang then Creator.Language = newLang end
...
function Creator.New(Name, Properties, Children)
	...
	for N, V in next, Properties or {} do
		if N ~= "ThemeTag" then
			Object[N] = V
		end
		if Creator.Localization and Creator.Localization.Enabled and N == "Text" then
			local TranslationId = string.match(V, "^" .. Creator.Localization.Prefix .. "(.+)")
...
-- ═══════════════════════════════════════════════════════════════
--  Creator.Tween  — standard tween factory (backward-compat)
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
--  Creator.SpringTween  — Leviathan spring-style tween helper
--  Returns the Tween object (caller must :Play())
-- ═══════════════════════════════════════════════════════════════
function Creator.SpringTween(Object, Properties, Duration)
	Duration = Duration or Creator.Anim.Spring
	return TweenService:Create(
		Object,
		TweenInfo.new(Duration, Enum.EasingStyle.Spring, Enum.EasingDirection.Out, 0, false, 0),
		Properties
	)
end

-- ═══════════════════════════════════════════════════════════════
--  Creator.QuickTween  — fire-and-forget convenience helper
--  Plays immediately; returns nothing.
--  Creator.QuickTween(obj, 0.15, {ImageTransparency = 0})
-- ═══════════════════════════════════════════════════════════════
function Creator.QuickTween(Object, Time, Properties, EasingStyle, EasingDirection)
...
-- ═══════════════════════════════════════════════════════════════
--  NewRoundFrame  — delegates to DynamicShape (backward-compat)
-- ═══════════════════════════════════════════════════════════════
...
-- local aliases used internally after this point
local New   = Creator.New
local Tween = Creator.Tween

-- ═══════════════════════════════════════════════════════════════
--  Drag system  (100% backward-compatible)
-- ═══════════════════════════════════════════════════════════════
...
function Creator.Drag(mainFrame, dragFrames, ondrag)
	...
	for _, dragFrame in pairs(dragFrames) do
		dragFrame.InputBegan:Connect(function(input)
			if not DragModule.CanDraggable or dragging then return end
			if input.UserInputType == Enum.UserInputType.MouseButton1
				or input.UserInputType == Enum.UserInputType.Touch
			then
				if WindUI and WindUI.CurrentInput and WindUI.CurrentInput ~= CurInput then return end
	...
	UserInputService.InputEnded:Connect(function(input)
		if not dragging or WindUI.CurrentInput ~= CurInput then return end
...
-- Initialise Icons image builder
Icons.Init(New, "Icon")
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
	...
	elseif string.find(Img, "http") and not string.find(Img, "roblox.com") then
		local FileName = "LeviathanUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
		local success = pcall(function()
			task.spawn(function()
				local body = Creator.Request
					and Creator.Request({ Url = Img, Method = "GET" }).Body
					or {}
...
-- ═══════════════════════════════════════════════════════════════
--  Leviathan: LerpColor  — lerp two Color3s by t ∈ [0,1]
-- ═══════════════════════════════════════════════════════════════
function Creator.LerpColor(a, b, t)
	t = math.clamp(t, 0, 1)
	return Color3.new(
		a.R + (b.R - a.R) * t,
		a.G + (b.G - a.G) * t,
...
-- ═══════════════════════════════════════════════════════════════
--  Leviathan: AdjustBrightness  — lighten (+) or darken (-)
--  amount ∈ [-1, 1]
-- ═══════════════════════════════════════════════════════════════
function Creator.AdjustBrightness(color, amount)
	return Color3.new(
...
-- ═══════════════════════════════════════════════════════════════
--  AddColor  — additive colour modifier for ThemeTags
--  Returns a function(theme) → Color3
-- ═══════════════════════════════════════════════════════════════
function Creator:AddColor(base, add, weight)
	...
	return function(theme)
		...
		if not baseColor or typeof(baseColor) ~= "Color3" then return nil end
		return Color3.new(
...
-- ═══════════════════════════════════════════════════════════════
--  GetElementPosition  — squircle grouping helper
--  Returns shape-type string for an element in a list,
--  accounting for Divider/Space/Section delimiters.
-- ═══════════════════════════════════════════════════════════════
function Creator:GetElementPosition(elements, targetIndex, isHStack)
	...
	if isDelimiter(elements[targetIndex]) then return nil, 3 end
...
-- ═══════════════════════════════════════════════════════════════
--  Leviathan: HoverEffect  — attach hover highlight to any frame
--
--  Usage:
--    local cleanup = Creator.HoverEffect(frame, {
--        HoverTransparency = 0.88,    -- ImageTransparency when hovered
--        NormalTransparency = 1,      -- ImageTransparency at rest
--        Duration = 0.10,             -- tween time
--    })
--    cleanup()  -- disconnect when element is destroyed
-- ═══════════════════════════════════════════════════════════════
function Creator.HoverEffect(Object, Config)
	Config = Config or {}
	local hoverT  = Config.HoverTransparency  ~= nil and Config.HoverTransparency  or 0.88
	local normalT = Config.NormalTransparency ~= nil and Config.NormalTransparency or 1
	local dur     = Config.Duration or Creator.Anim.Fast
	local prop    = Config.Property or "ImageTransparency"

	local c1 = Object.MouseEnter:Connect(function()
		Creator.QuickTween(Object, dur, { [prop] = hoverT })
	end)
	local c2 = Object.MouseLeave:Connect(function()
		Creator.QuickTween(Object, dur, { [prop] = normalT })
	end)
...
-- ═══════════════════════════════════════════════════════════════
--  Leviathan: PressEffect  — attach press scale/alpha to a button
--
--  Usage:
--    Creator.PressEffect(button, frame)
-- ═══════════════════════════════════════════════════════════════
function Creator.PressEffect(Button, VisualFrame, Config)
	Config = Config or {}
	local pressScale  = Config.PressScale  or 0.97
	local normalScale = Config.NormalScale or 1.00
	local dur         = Config.Duration    or Creator.Anim.Fast

	-- Attach a UIScale if not present
	local uiScale = VisualFrame:FindFirstChildOfClass("UIScale")
	if not uiScale then
		uiScale = Instance.new("UIScale")
		uiScale.Scale  = normalScale
		uiScale.Parent = VisualFrame
	end

	Button.MouseButton1Down:Connect(function()
		Creator.QuickTween(uiScale, dur, { Scale = pressScale },
			Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
	end)
	Button.MouseButton1Up:Connect(function()
		Creator.QuickTween(uiScale, dur * 1.5, { Scale = normalScale },
			Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	end)
	Button.MouseLeave:Connect(function()
		Creator.QuickTween(uiScale, dur, { Scale = normalScale },
			Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
	end)
end
