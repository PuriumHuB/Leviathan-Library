-- Leviathan UI — Tag (topbar tag via Window:Tag())
-- Fixed: nil TagIcon crash in OnThemeChange
...
function Tag:New(TagConfig, Parent)
	...
	local TagTitle = New("TextLabel", {
		...
		TextColor3 = typeof(TagModule.Color) == "Color3"
			and Creator.GetTextColorForHSB(TagModule.Color)
			or typeof(TagModule.Color) == "string"
			and (Creator.GetTextColorForHSB(Creator.GetThemeProperty(TagModule.Color, Creator.Theme)))
			or Color3.new(1, 1, 1),
	...
	if typeof(TagModule.Color) == "table" then
		...
		local avg = Creator.GetAverageColor(BackgroundGradient)
		TagTitle.TextColor3 = Creator.GetTextColorForHSB(avg)
		if TagIcon then
			TagIcon.ImageLabel.ImageColor3 = Creator.GetTextColorForHSB(avg)
		end
	end
	...
	function TagModule:SetColor(color)
		...
		if typeof(color) == "table" then
			...
			if BackgroundGradient then BackgroundGradient:Destroy() end
	...
	function TagModule:SetIcon(icon)
		...
		if icon then
			...
			TagIcon.Size = UDim2.new(0, TagModule.IconSize, 0, TagModule.IconSize)
			TagIcon.Parent = TagFrame.Content
			...
			elseif typeof(TagModule.Color) == "table" and BackgroundGradient then
				TagIcon.ImageLabel.ImageColor3 = Creator.GetTextColorForHSB(
					Creator.GetAverageColor(BackgroundGradient)
				)
			end
		else
	...
	-- Fixed: guard nil TagIcon before accessing .ImageLabel
	Creator:OnThemeChange(function(NewTheme, OldTheme)
		if typeof(TagModule.Color) == "string" then
			local c = Creator.GetThemeProperty(TagModule.Color, Creator.Theme)
			if c then
				TagTitle.TextColor3 = Creator.GetTextColorForHSB(c)
				if TagIcon then
					TagIcon.ImageLabel.ImageColor3 = Creator.GetTextColorForHSB(c)
				end
			end
		end
	end)
